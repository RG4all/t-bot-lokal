...
Release **2.4.15** wurde lokal mit **279 Django-/Python-Tests**, **8/8 Shell-Testgruppen**, Ruff, ShellCheck, Systemcheck, Migrationsprüfung, `collectstatic` und `pip check` validiert; zusätzlich lief die Ressourcen-Probe `scripts/backtest_resource_probe.py`. Docker-/Compose-Laufzeitprüfungen wurden mangels Docker übersprungen. Im Repository ist kein GitHub-Actions-Anwendungstestworkflow versioniert; für dieses Release wurde die Auslieferung mit erfolgreichen lokalen Prüfnachweisen ausdrücklich freigegeben. Die Dependency-Graph-Integration allein ersetzt keine Anwendungstests. Testergebnisse, CI-Ausnahme und Prüfgrenzen des Indikator-Refactorings: [CODE-18](CODE-18-indicator-dedup.md); Batch-Prüfgrenzen des DataLog-Trims: [PERF-17](PERF-17-db-trim-batch-delete.md). Die folgenden Kommandos dienen der lokalen Nachprüfung; Docker-Smoke-Tests und `pip-audit` sind zusätzliche, umgebungsabhängige Prüfungen.
