...
Für Release 2.4.9 wurden die folgenden QA-Kommandos lokal ausgeführt. Im Repository ist derzeit kein GitHub-Actions-Testworkflow versioniert; automatische Dependency-Graph-Läufe sind kein Nachweis für Anwendungstests. Die Grenzen der lokalen Prüfung stehen im [Security-Nachweis](SEC-09-permissions-policy.md).
...
# Gezielte Security-Regressionen (nosniff, CSRF-Cookie, Session, Cache-Control und Permissions-Policy):
python manage.py test trading.tests.test_content_type_nosniff trading.tests.test_csrf_cookie trading.tests.test_session_invalidate trading.tests.test_cache_control trading.tests.test_permissions_policy --noinput
python manage.py collectstatic --noinput
python -m pip check
