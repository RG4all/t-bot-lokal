...
Release **2.4.14** wurde lokal mit **260 Django-/Python-Tests**, **8/8 Shell-Testgruppen**, Ruff, ShellCheck, Systemcheck, Migrationsprüfung, `collectstatic` und `pip check` validiert. Der Indikator-Mikrobenchmark misst **1,31×** schnellere warme Cache-Zugriffe; dies ist keine End-to-End-Zusage. Docker-/Compose-Laufzeitprüfungen wurden mangels Docker übersprungen. Im Repository ist kein GitHub-Actions-Anwendungstestworkflow versioniert; die Dependency-Graph-Integration allein ersetzt keine Anwendungstests. Nachweise und aktuelle CI-Prüfgrenze: [PERF-16](PERF-16-indicator-memoization.md). Die folgenden Kommandos dienen der lokalen Nachprüfung; Docker-Smoke-Tests und `pip-audit` sind zusätzliche, umgebungsabhängige Prüfungen.
...
# Gezielte Indikator-Cache-Regressionen und Mikrobenchmark:
python manage.py test trading.tests.test_indicator_memoization --noinput
python scripts/benchmark_indicator_cache.py
# Gezielte Security-/Bug-Regressionen einschließlich des CSV-Exports:
python manage.py test trading.tests.test_content_type_nosniff trading.tests.test_csrf_cookie trading.tests.test_session_invalidate trading.tests.test_cache_control trading.tests.test_permissions_policy trading.tests.test_error_disclosure trading.tests.test_bot_start_stop trading.tests.test_report_csv --noinput
python manage.py collectstatic --noinput
python -m pip check
...
- **Backtesting-Cache (ab 2.4.14):** Ein threadsicherer LRU mit maximal 8.192 Einträgen vermeidet wiederholte Decimal-Indikatorberechnungen und wird bei Task-Ende auch nach Abbruch oder Fehler geleert. Formeln und vorberechnetes Kandidatenraster bleiben erhalten. [Finding, Messung und Prüfgrenzen](PERF-16-indicator-memoization.md).
