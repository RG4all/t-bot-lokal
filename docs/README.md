...
Release **2.4.17** wurde lokal mit **320 Django-/Python-Tests**, einem **mypy-Lauf ohne Befund**, **8/8 Shell-Testgruppen**, Ruff, ShellCheck, Systemcheck, Migrationsprüfung, `collectstatic` und `pip check` validiert. Docker-/Compose-Laufzeitprüfungen wurden mangels Docker übersprungen; `pyright` wurde nicht ausgeführt (Node nicht eingerichtet), das Kriterium der statischen Prüfung ist über mypy erfüllt. Im Repository ist kein GitHub-Actions-Anwendungstestworkflow versioniert; für dieses Release wurde die Auslieferung mit erfolgreichen lokalen Prüfnachweisen ausdrücklich freigegeben. Die Dependency-Graph-Integration allein ersetzt keine Anwendungstests. Testergebnisse, Negativkontrolle der Typprüfung, CI-Ausnahme und Prüfgrenzen: [CODE-19](CODE-19-view-type-hints.md); Indikator-Refactoring: [CODE-18](CODE-18-indicator-dedup.md); Batch-Prüfgrenzen des DataLog-Trims: [PERF-17](PERF-17-db-trim-batch-delete.md); Modul-Exports: [CODE-20](CODE-20-module-exports.md). Die folgenden Kommandos dienen der lokalen Nachprüfung; Docker-Smoke-Tests, `mypy` und `pip-audit` sind zusätzliche, umgebungsabhängige Prüfungen.
