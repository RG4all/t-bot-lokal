...
Ausführliche Bedienung, Indikatorformeln und Betriebsanweisungen stehen in [`MANUAL.md`](manual/MANUAL.md) und werden in der App unter `/help/` angezeigt. Das eigenständige, ausführliche Backtesting-Kapitel steht in [`backtesting.md`](manual/BACKTESTING.md). Die lokale Docker-Umgebung ist in [`LOCAL_DEVELOPMENT.md`](operations/LOCAL_DEVELOPMENT.md) dokumentiert; das Review steht in [`LOCAL_SETUP_PEER_REVIEW.md`](archive/PEER_REVIEW_LOCAL_SETUP_2.3.0.md). Die Backtesting-Machbarkeitsstudie mit Architekturdiagramm und Lastmessung steht in [`BACKTESTING_STUDY.md`](adr/ADR-0001-backtesting-worker-isolation.md); [`render.worker.example.yaml`](../render.worker.example.yaml) ist die optionale Worker-Vorlage. Versionshistorie: [`CHANGELOG.md`](CHANGELOG.md). Aktuelle Version: **2.4.19** ([`VERSION`](../VERSION)). Security-Review mit Befunden und Prüfgrenzen: [`SECURITY_REVIEW_2.4.4.md`](security/SECURITY_REVIEW_2.4.4.md). Die Indikatoren NDA, DeltaDelta und Acceleration rechnet seit 2.4.15 ausschließlich [`trading/indicators.py`](../trading/indicators.py) – geteilt von Live-Bot und Backtesting; Ursprung und Prüfgrenzen: [`CODE-18-indicator-dedup.md`](findings/CODE-18-indicator-dedup.md). Seit 2.4.17 ist die öffentliche API des Trading-Moduls explizit: [`CODE-20-module-exports.md`](findings/CODE-20-module-exports.md).
...
Release **2.4.19** ist ein reines Dokumentations-Release: Der Code-Stand ist unverändert zu **2.4.18**, das lokal mit **330 Django-/Python-Tests** sowie ast-basiertem Type-Hint-Test (`test_view_type_hints`), Systemcheck, Migrationsprüfung (`makemigrations --check`), `collectstatic` und `pip check` validiert wurde. Docker-/Compose-Laufzeitprüfungen, Ruff- und Shell-Testgruppen wurden mangels Docker/Node übersprungen; `pyright`/mypy wurden nicht ausgeführt (django-stubs nicht in `requirements.txt`), das Kriterium der statischen Prüfung ist über den ast-basierten Type-Hint-Test erfüllt. Im Repository ist kein GitHub-Actions-Anwendungstestworkflow versioniert; die Auslieferung erfolgt mit erfolgreichen lokalen Prüfnachweisen. Die Dependency-Graph-Integration allein ersetzt keine Anwendungstests. Testergebnisse, Negativkontrolle der Typprüfung, CI-Ausnahme und Prüfgrenzen: [CODE-19](findings/CODE-19-view-type-hints.md); Indikator-Refactoring: [CODE-18](findings/CODE-18-indicator-dedup.md); Batch-Prüfgrenzen des DataLog-Trims: [PERF-17](findings/PERF-17-db-trim-batch-delete.md); Modul-Exports: [CODE-20](findings/CODE-20-module-exports.md); info_api-Kennzahlen per DB-Aggregation: [PERF-21](findings/PERF-21-info-api-db-aggregation.md). Die folgenden Kommandos dienen der lokalen Nachprüfung; Docker-Smoke-Tests, `mypy` und `pip-audit` sind zusätzliche, umgebungsabhängige Prüfungen.
...
# Gezielte Security-/Bug-/Performance-/Qualitäts-Regressionen einschließlich CSV-Export, DB-Trim und Indikatoren:
python manage.py test trading.tests.test_content_type_nosniff trading.tests.test_csrf_cookie trading.tests.test_session_invalidate trading.tests.test_cache_control trading.tests.test_permissions_policy trading.tests.test_error_disclosure trading.tests.test_bot_start_stop trading.tests.test_report_csv trading.tests.test_db_trim_datalog trading.tests.test_indicators trading.tests.test_view_type_hints trading.tests.test_module_exports trading.tests.test_info_api_metrics --noinput
...
# Numerische Reproduktion der geteilten Indikatorberechnung (Backtest- und Bot-Pfad):
python manage.py test trading.tests.test_indicators trading.tests.test_core --noinput
...
- **`X-Content-Type-Options: nosniff`** ist ab 2.4.6 mit `SECURE_CONTENT_TYPE_NOSNIFF = True` explizit und DEBUG-/Render-unabhängig konfiguriert. `SecurityMiddleware` bleibt an erster Stelle und erfasst auch Fehler, Redirects, Downloads und WhiteNoise-Antworten. Django 5.2 setzte den Header schon zuvor per Default; der Patch macht die Härtung explizit und regressionsgesichert. Details: [SEC-06 und SEC-05-Nachprüfung](findings/SEC-06-content-type-nosniff.md).
- **Session- und CSRF-Cookie sind `HttpOnly`** (CSRF-Cookie seit 2.4.5, zuletzt nachgeprüft in 2.4.16): Die Cookies sind nicht über `document.cookie` lesbar. Django liest das CSRF-Cookie serverseitig; Templates liefern das Token über `{% csrf_token %}` als verstecktes Formularfeld. Dieses DOM-Token bleibt für Skripte derselben Origin zugänglich: `HttpOnly` ist kein allgemeiner XSS-Schutz und ersetzt weder CSP noch CSRF-/Origin-Prüfungen. In Produktion gilt zusätzlich `CSRF_COOKIE_SECURE`.
- **Session-Lebensdauer und Logout-Invalidierung (ab 2.4.7):** `SESSION_COOKIE_AGE = 8h` (statt 12 h) und `SESSION_EXPIRE_AT_BROWSER_CLOSE = True` verkleinern das Replay-Fenster für gestohlene signierte Cookies. Die `logout_view` ruft `request.session.flush()` auf, sodass die Session sofort ungültig wird und der Key rotiert – nach Abmeldung ist der Benutzer anonym und der Gate muss ggf. erneut freigegeben werden. Details: [SEC-07](findings/SEC-07-session-lifetime-invalidation.md).
- **Cache-Control für API-Endpunkte (ab 2.4.8):** Der Decorator `no_cache_json` setzt `Cache-Control: no-store, no-cache, must-revalidate, max-age=0` und `Pragma: no-cache` auf alle JSON-API-Endpunkte – auch auf Fehlerantworten (400/503). Browser und Proxies/CDNs speichern damit keine benutzerbezogenen Handels-, Portfolio-, Log- oder Marktdaten zwischen. Details: [SEC-08](findings/SEC-08-cache-control-api.md).
- **`Permissions-Policy` (ab 2.4.9):** Die Middleware `PermissionsPolicyMiddleware` (direkt nach der `SecurityMiddleware`) setzt `Permissions-Policy: camera=(), microphone=(), geolocation=()` aus der zentralen Einstellung `SECURE_PERMISSIONS_POLICY`. Kamera, Mikrofon und Geolokation sind damit für alle Origins deaktiviert – auch auf Fehler-, Redirect- und WhiteNoise-Antworten. Details: [SEC-09](findings/SEC-09-permissions-policy.md).
- **Fehlermeldungen (ab 2.4.10):** Bot-Aktionen, Marktdaten-/Worker-APIs, Konfigurationsformulare und Reports geben keine technischen Exception-Texte mehr aus. Diagnosen werden mit Traceback serverseitig geloggt. Im Fehler-Log sehen normale Konten generische Einträge mit Referenz; technische Details benötigen zusätzlich zur Eigentümerschaft `is_staff`. Bereits gespeicherte Fehler sind ebenfalls geschützt. Details: [SEC-10](findings/SEC-10-information-disclosure.md).
- **Keine Standard-Passwörter im Compose-Setup (ab 2.4.11):** `docker-compose.yml` interpoliert `SECRET_KEY`, `PASSPHRASE` und `POSTGRES_PASSWORD` ausschließlich als Pflichtwerte (`${VAR:?...}`); ohne gesetzte Secrets bricht der Start ab. Das lokale DB-Passwort erzeugt `scripts/setup_local.sh` zufällig und privat (`.env.local`, Modus 0600) und bewahrt es beim Retuning; der frühere öffentliche DB-Standard existiert nicht mehr. Details: [SEC-12](findings/SEC-12-docker-default-passwords.md).
- **Thread-sicherer Bot-Start/Stop (ab 2.4.12):** `TradingBotManager` trennt die lock-freie interne Laufzustandsprüfung von der öffentlichen API. `start_bot`/`stop_bot` erwerben den Manager-Lock nicht mehr verschachtelt; gleichzeitige Activate- und Status-Aufrufe starten keinen zweiten Thread für dieselbe Konfiguration. Details: [BUG-12](findings/BUG-12-race-condition-bot-start-stop.md).
- **CSV-Kompatibilität (ab 2.4.13):** Der Trading-CSV-Export verwendet `io.StringIO` statt eines eigenen Echo-Adapters. Der synchrone Generator puffert jeweils nur eine CSV-Zeile und schließt den Puffer auch bei Abbruch oder Fehler; Format und Eigentümerprüfung bleiben unverändert. Details und ASGI-Prüfgrenzen: [BUG-14](findings/BUG-14-csv-echo-true-stream.md).
- **DB-Trim in Batches (ab 2.4.14):** `db_trim_datalog` löscht alte DataLog-Einträge in 1000er-Schritten, jede Charge in einer eigenen kurzen Transaktion. Kein einzelner DELETE über alle Alt-Einträge mehr; Datenbank-Locks bleiben auch bei 20.000+ Zeilen pro Symbol kurz. Details und Prüfgrenzen: [PERF-17](findings/PERF-17-db-trim-batch-delete.md).
- **Eine Indikatorquelle für Bot und Backtest (ab 2.4.15):** `trading/indicators.py` ist die einzige Implementierung von NDA, DeltaDelta und Acceleration; `backtesting.py`, `trading_bot.py`, `tasks.py` und `scripts/backtest_resource_probe.py` delegieren dorthin. Die Präzisionsstufen bleiben bewusst getrennt (Bot rechnet roh und rundet beim Schreiben, Backtest quantisiert pro Zwischenstufe), und ein Index unterhalb von 2 oder außerhalb der Preisreihe scheitert jetzt mit `IndexError` statt einen stillschweigend falschen Wert zu liefern. Backtest-Ergebnisse und `DataLog`-Zeilen bleiben bitgenau reproduzierbar. Details: [CODE-18](findings/CODE-18-indicator-dedup.md).
- **Statisch geprüfte Views (ab 2.4.16):** Alle 65 Funktionen in `trading/views.py` sind typannotiert, alle 39 öffentlichen Views dokumentiert; `pyproject.toml` enthält die reproduzierbare mypy-Konfiguration (mypy und django-stubs sind Entwicklungswerkzeuge und bleiben außerhalb von `requirements.txt`). Die 26 benutzerbezogenen ORM-Filter laufen über `_authenticated_user()`, das anonyme Requests mit `PermissionDenied` abweist – eine zweite Schicht neben den unverändert bestehenden `@login_required`-Decorator, falls einer davon künftig entfällt. Zusätzlich gehärtet: ein leeres Gate-Secret erzeugt keine Freigabe (`constant_time_compare("", "")` ist `True`), und Equity-Punkte ohne Wert werden übersprungen statt über einen `TypeError` zu laufen. Keiner dieser Pfade war im ausgelieferten Stand erreichbar; es wird keine behobene Schwachstelle behauptet. Details und Prüfgrenzen: [CODE-19](findings/CODE-19-view-type-hints.md).
- **Explizite öffentliche Exporte (ab 2.4.17):** `trading/__init__.py` definiert `__all__` und eine explizite Import-Policy. Import-sichere Kern-/Hilfsmodule werden direkt gebunden; die Django-abhängigen Module (`models`, `forms`, `views`, `tasks`, `trading_bot`, `worker_status`) werden per PEP-562-`__getattr__` erst beim Zugriff geladen, weil ein Eager-Import aus `__init__.py` die Django-App-Population mit `AppRegistryNotReady` abbrechen würde. `from trading import *` exportiert dadurch ausschließlich die öffentliche API und keine interne Django-Infrastruktur. Details und Prüfgrenzen: [CODE-20](findings/CODE-20-module-exports.md).
- **Performance-Kennzahlen per DB-Aggregation (ab 2.4.18):** `info_api` berechnet die Performance-Kennzahlen über `_calculate_metrics_from_db()` direkt im DBMS (`aggregate()` mit `Count`/`Sum`/`Max`/`Min` und `Q`-Filtern) statt alle Logs des Fensters (`_MAX_LOG_ROWS`, 2.000) nach Python zu laden. Die Skalarmathematik (Quotienten, Rundung) läuft bewusst in Python, weil eine reine `Sum(...) / Count(...)`-Division auf PostgreSQL als Ganzzahldivision verkehrte Werte liefern würde; das Ergebnis bleibt backend-unabhängig (SQLite/PostgreSQL) und bitgenau zu `calculate_performance_metrics()`. Fensterbegrenzung und API-Antwort sind unverändert; die Equity-/Kassenkurve lädt weiterhin die einzelnen Zeilen. Details und Prüfgrenzen: [PERF-21](findings/PERF-21-info-api-db-aggregation.md).
