...
Release **2.4.18** wurde lokal mit **330 Django-/Python-Tests**, dem ast-basierten Type-Hint-Test (`test_view_type_hints`), Systemcheck, Migrationsprüfung (`makemigrations --check`), `collectstatic` und `pip check` validiert. Docker-/Compose-Laufzeitprüfungen, Ruff- und Shell-Testgruppen wurden mangels Docker/Node übersprungen; `pyright`/mypy wurden nicht ausgeführt (django-stubs nicht in `requirements.txt`), das Kriterium der statischen Prüfung ist über den ast-basierten Type-Hint-Test erfüllt. Im Repository ist kein GitHub-Actions-Anwendungstestworkflow versioniert; für dieses Release wurde die Auslieferung mit erfolgreichen lokalen Prüfnachweisen ausdrücklich freigegeben. Die Dependency-Graph-Integration allein ersetzt keine Anwendungstests. Testergebnisse, Negativkontrolle der Typprüfung, CI-Ausnahme und Prüfgrenzen: [CODE-19](CODE-19-view-type-hints.md); Indikator-Refactoring: [CODE-18](CODE-18-indicator-dedup.md); Batch-Prüfgrenzen des DataLog-Trims: [PERF-17](PERF-17-db-trim-batch-delete.md); Modul-Exports: [CODE-20](CODE-20-module-exports.md); info_api-Kennzahlen per DB-Aggregation: [PERF-21](PERF-21-info-api-db-aggregation.md). Die folgenden Kommandos dienen der lokalen Nachprüfung; Docker-Smoke-Tests, `mypy` und `pip-audit` sind zusätzliche, umgebungsabhängige Prüfungen.
