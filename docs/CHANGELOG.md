...
Bugfix- und Fehler-UX-Release; setzt das [Code-Review vom 2026-09-09](security/CODE_REVIEW_2026-09-09.md) für Exchange-Anbindung und Fehler-Log fort. Drei Befunde (jeder ein eigener Commit mit Regressionstest, rot am Ausgangsstand): [BUG-26](findings/BUG-26-bitunix-spot-scanner-empty.md), [BUG-27](findings/BUG-27-empty-timeout-error-log.md), [CODE-28](findings/CODE-28-generic-error-messages.md). Teststand nach dem Release: 393 Django-/Python-Tests grün, Ruff und mypy ohne Befund.

### Fixed

- **Bitunix-Spot-Marktscan lieferte nie qualifizierte Märkte (BUG-26):** Die dokumentierte Bitunix-Spot-Kline-API enthält keine 24h-Volumendaten, deshalb schieden beim Top-Gainer/-Loser-Scan (Vorlagen und Analyse) alle Spot-Märkte bei jedem Volumen-Filter aus. Wenn die Exchange kein Quotevolumen liefert, misst der Scan jetzt die **absolute Orderbuch-Tiefe** (Boden 100.000 USDT innerhalb ±2 % um den Mittelkurs) als Liquiditätsnachweis; die Volumen-Kriterien werden transparent als „nicht ermittelt" angezeigt. Zusätzlich liefert die Scan-API `excluded_sample` (bis zu 5 nächste Ausschlüsse mit Hauptgrund), das leere Frontend zeigt diese, und der 503-Fehler trägt eine serverseitig formulierte `user_message` (generische Zeile bleibt Fallback, keine Exception-Texte). Dazu im selben Befundkomplex: Bitunix-Spot-Paare **ohne** Statusfeld gelten nicht als handelbar (fail-closed statt stillschweigender Freigabe), `validate_symbols` liefert konsistent `[]`, und Spot-Einzelabfragen werden mit 0,11 s getaktet (bleibt unter dem dokumentierten 10-Requests/Sek/IP-Limit).
- **Kursabruf-Timeout erzeugte leere Fehler-Log-Einträge und 2-s-Retry (BUG-27):** `asyncio.wait_for` warf bei Ablauf ein `TimeoutError`, dessen `str()` der leere String ist – die Meldung landete leer im Fehler-Log (im UI nur „Referenz #N"), und der Backoff klassifizierte den Fall als „sonstiges" mit 2-s-Wiederholung gegen eine langsame/gesperrte Exchange. Der Fetch-Timeout ist jetzt `MarketDataTimeoutError` (Subtyp von `MarketDataConnectionError`) mit fester Meldung (Börse, Symbolanzahl, Budget) und erbt damit den 5–300-s-Backoff; das Budget skaliert mit der Symbolzahl (Basis 20 s + 12 s je Symbol, max. 120 s). Alle `ErrorLog`-Pfade (db_log_error, persist_error, Main-Loop, run-Catchall) fallen bei meldungslosen Ausnahmen auf einen nicht-leeren Fallback zurück, der den Exception-Typ benennt.
- **Scanner-503 ohne nutzbare Information (Teil von BUG-26):** Der Marktscan-503 zeigte immer dieselbe generische Zeile, auch wenn der Scanner den Grund kannte (z. B. „keine Märkte gefunden"). Bekannte `MarketScannerError`-Varianten tragen jetzt eine serverseitig formulierte Kurzbeschreibung, die in die Antwort fließt; Exception-Texte bleiben ausgeschlossen (SEC-10-Nachweis).

### Added

- **Menschenlesbare Fehler-Erklärungen im Fehler-Log (CODE-28):** Neue Karte `trading/error_explanations.py` erklärt **alle 12** ErrorLog-Quellen in Was-ist-passiert / Ursache / Was-tun; jede Zeile im Fehler-Log zeigt zusätzlich eine **Referenznummer** (Eintrag im Server-Log) und sichere Kontextdaten (Exchange, betroffene Symbole, Ban-Zeitpunkt) – freigegeben ausschließlich aus einer Whitelist, rohe Meldungen/Tracebacks bleiben staff-only (SEC-10). Unbekannte Quellen erhalten einen ehrlichen Generik-Text mit Verweis auf die Referenznummer; ein Quell-Scan-Test zwingt neue Fehlerquellen zu einer Erklärung.
- **23 Regressionstests** (`trading/tests/test_bitunix_error_handling.py`): dokumentierte Bitunix-Response-Formen, fail-closed Katalog, Symbol-Pacing, Spot-Scan mit/ohne Orderbuch-Tiefe, Timeout-Kette, leere-Meldung-Fallbacks, Erklärungs-Komplettität.
