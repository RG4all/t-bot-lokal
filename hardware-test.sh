...
compute_recommendations() {
  ...
  # Compose-Cgroups und Backtest-Budget an denselben RAM-Snapshot koppeln.
  # Diese Werte sind bewusst konservativer als die physische Kapazität, damit
  # PostgreSQL, Redis und der Trading-Thread gemeinsam Luft behalten.
  if (( total_mb >= 12288 )); then
    WEB_MEMORY=1536m; WORKER_MEMORY=1024m; POSTGRES_MEMORY=1024m
    REDIS_MEMORY=256m; SCHEDULER_MEMORY=256m
    BACKTEST_MAX_COMBINATIONS=20000; BACKTEST_MAX_PRICE_POINTS=5000
  elif (( total_mb >= 6144 )); then
    WEB_MEMORY=1024m; WORKER_MEMORY=768m; POSTGRES_MEMORY=512m
    REDIS_MEMORY=128m; SCHEDULER_MEMORY=192m
    BACKTEST_MAX_COMBINATIONS=12000; BACKTEST_MAX_PRICE_POINTS=5000
  elif (( total_mb >= 2048 )); then
    WEB_MEMORY=512m; WORKER_MEMORY=512m; POSTGRES_MEMORY=256m
    REDIS_MEMORY=64m; SCHEDULER_MEMORY=128m
    BACKTEST_MAX_COMBINATIONS=5000; BACKTEST_MAX_PRICE_POINTS=2500
  else
    WEB_MEMORY=384m; WORKER_MEMORY=384m; POSTGRES_MEMORY=192m
    REDIS_MEMORY=48m; SCHEDULER_MEMORY=96m
    BACKTEST_MAX_COMBINATIONS=2000; BACKTEST_MAX_PRICE_POINTS=1000
  fi
  BACKTEST_DEFAULT_PRICE_POINTS="${BACKTEST_MAX_PRICE_POINTS}"
...
BACKTEST_MAX_COMBINATIONS=${BACKTEST_MAX_COMBINATIONS}
BACKTEST_MAX_PRICE_POINTS=${BACKTEST_MAX_PRICE_POINTS}
BACKTEST_DEFAULT_PRICE_POINTS=${BACKTEST_DEFAULT_PRICE_POINTS}
WEB_CPUS=${WEB_CPUS}
...
------------------------
  ...
  t-bot / App
    ...
    Backtest-Kombinationen: ${BACKTEST_MAX_COMBINATIONS}
    Backtest-Preispunkte : ${BACKTEST_MAX_PRICE_POINTS}
